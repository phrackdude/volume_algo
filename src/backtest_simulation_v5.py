"""
Volume Cluster Trading Strategy - Version 5.0 (Tightened Adaptive)
...
- Focus on top-1 volume clusters only
...
TOP_N_CLUSTERS_PER_DAY = 1      # Focus on top-1 only (was 2)
...
def save_statistics_to_csv(modal_stats_history, volume_stats_history):
    ...
    if not modal_df.empty:
        modal_df.to_csv("../data/modal_position_returns_v5.csv", index=False)
        print(f"Saved V5 modal position statistics: {len(modal_df)} bin-date combinations")
    ...
    if not volume_df.empty:
        volume_df.to_csv("../data/volume_ratio_returns_v5.csv", index=False)
        print(f"Saved V5 volume ratio statistics: {len(volume_df)} trading days")
...
print("Starting V5 Tightened Adaptive backtest simulation...")
print(f"Key V5 optimizations:")
print(f"  - Top {TOP_N_CLUSTERS_PER_DAY} cluster(s) only")
...
for day, group in tqdm(df.groupby(df.index.date), desc="Processing days"):
    ...
    # V5: Focus on TOP-1 clusters only
    ...
    for cluster_idx, cluster_info in enumerate(top_n_clusters):
        ...
        # Calculate volatility and targets (same as V4)
...
result_df.to_csv("../data/backtest_results_v5.csv", index=False)
...
# Enhanced Analysis
if len(result_df) > 0:
    print(f"\n=== V5 TIGHTENED ADAPTIVE STRATEGY RESULTS ===")
    ...
    # Volume focus analysis (should be all rank 1 now)
    ...
    print(f"\nFILES EXPORTED:")
    print(f"  backtest_results_v5.csv - Main V5 backtest results")
    print(f"  modal_position_returns_v5.csv - V5 Modal position bin statistics")
    print(f"  volume_ratio_returns_v5.csv - V5 Volume ratio statistics")
    
else:
    print("No trades found in V5 tightened adaptive simulation.") 